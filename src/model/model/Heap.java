...
public class Heap<V, P extends Comparable<P>> implements IPriorityQueue<V, P> {
    ...
    public void maxHeapify(int index) {
        ...
        if (left < size && heap[left(index)].compareTo(heap[index]) > 0) {
            largest = left;
        }
        if (right < size && heap[right(index)].compareTo(heap[largest]) > 0) {
            largest = right;
        }
        if (largest != index) {
            swap(index, largest);
    ...
    public void minHeapify(int index) {
        ...
        if (right < size && heap[right(index)].compareTo(heap[smallest]) < 0) {
            smallest = right;
        }
        if (smallest != index) {
            swap(index, smallest);
